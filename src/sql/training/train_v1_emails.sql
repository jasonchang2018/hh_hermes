...
create or replace temporary table edwprodhh.pub_mbutler.temporary_raw_dataset as (
    ...
    Emails as (
        select 
            emid_idx as Email_idx, 
            status_date,
            row_number() over (partition by em.debtor_idx order by status_date) as frequency, 
            em.debtor_idx, 
            previous_contacts, 
            dol_commission_attr
        from edwprodhh.pub_jchang.master_Emails as em
        inner join previous_contact_nums as co 
            on em.debtor_idx = co.debtor_idx 
            and em.emid_idx = co.contact_id
        left join commission_attributions as attr 
            on em.emid_idx = attr.contact_id
    ),
    all_contacts as (
        select 
            debtor_idx,
    ...
    full_dataset as (
        ...
        on em.debtor_idx = debtor.debtor_idx 
        inner join previous_contact_nums_by_type as p
        on em.debtor_idx = p.debtor_idx 
        and em.email_idx = p.contact_id 
    )
    select * 
    from full_dataset 
) 
...
select * 
from edwprodhh.pub_mbutler.emails_reserve 
; 


/* below creates the final table that you are going to pull into your model */ 
;
create or replace table edwprodhh.pub_mbutler.contact_strategy_emails_6 as (
    with raw_data as (
        select *
...
select      
            ...
            experian_score,
            dol_commission_attr
from stored_observations where pn < .90 
)  
;