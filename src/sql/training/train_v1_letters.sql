...
create or replace temporary table edwprodhh.pub_mbutler.temporary_raw_dataset as (
    ...
    Letters as (
        select 
            letter_id as Letter_idx, 
            expected_sent_date,
            row_number() over (partition by l.debtor_idx order by expected_sent_date) as frequency, 
            l.debtor_idx, 
            previous_contacts,  
            dol_commission_attr
        from edwprodhh.pub_jchang.master_Letters as l
        inner join previous_contact_nums as co 
            on l.debtor_idx = co.debtor_idx 
            and letter_idx = co.contact_id
        left join commission_attributions as attr 
            on letter_idx = attr.contact_id
    ),
    all_contacts as (
        select 
            debtor_idx,
    ...
    full_dataset as (
        select     
            ...
            email, 
            assigned_date,  
            expected_sent_date - assigned_date as debt_age, 
            previous_contacts,
            eqb_score, 
            assigned_amt, 
            median_household_income, 
        ...
        on l.debtor_idx = debtor.debtor_idx 
        inner join previous_contact_nums_by_type as p 
        on p.debtor_idx = l.debtor_idx 
        and p.contact_id = l.letter_idx 
    )
    select * 
    from full_dataset 
) 
...
;
create or replace table edwprodhh.pub_mbutler.contact_strategy_letters_9 as (
    with raw_data as (
        select *
    ...
    trimmed_data as (
        select 
            ...
            percentile_cont(0.50) within group (order by assigned_amt) over () as assigned_amt_upper,
    ...
    applied_trim as (
        ...
        where 
            ...
            (previous_contacts is null or (previous_contacts between previous_contacts_lower and previous_contacts_upper)) 
...
select      contact_id, 
            ...
            experian_score,
            dol_commission_attr
from stored_observations where pn < 0.97 
) 
SELECT * 
FROM TEST 
) 
