...
create or replace temporary table edwprodhh.pub_mbutler.temporary_raw_dataset as (
    ...
    full_dataset as (
        ...
        on t.debtor_idx = p.debtor_idx 
        and t.text_idx = p.contact_id
    )
    select * 
    from full_dataset 
) 
...
create or replace table edwprodhh.pub_mbutler.contact_strategy_texts_6 as (
    with raw_data as (
        select *
    ...
    applied_trim as (
        select 
            ...
            median_household_income, 
            experian_score,
            (dol_commission_attr - min(dol_commission_attr) over()) / 
            (max(dol_commission_attr) over() - min(dol_commission_attr) over()) as dol_commission_attr, 
